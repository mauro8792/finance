/**
 * Production smoke P0.10 — no fictitious cards/payments.
 * Missing-card GETs/POSTs must 404; payment_links stay 0.
 */
...
async function main(): Promise<void> {
  ...
  const missingPayment = randomUUID();
  const smokeKey = `smoke-p10-${randomUUID()}`;
  ...
  if (cookieHeader) {
    ...
    await get(
      `/api/credit-cards/${missingCard}/payments`,
      "GET payments missing card → 404",
      404
    );
    await get(
      `/api/credit-cards/${missingCard}/payments/${missingPayment}`,
      "GET payment detail missing → 404",
      404
    );
    await post(
      `/api/credit-cards/${missingCard}/payments`,
      "POST payment missing card → 404",
      {
  ...
  console.log(JSON.stringify({ api: API, smokeKey, results }, null, 2));
