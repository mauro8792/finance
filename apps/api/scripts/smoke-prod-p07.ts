/**
 * Production smoke P0.7 (no writes): health + auth + MVP1 + purchases + commitments.
 * Usage: npx tsx scripts/smoke-prod-p07.ts
 */
...
async function main(): Promise<void> {
  ...
  if (cookieHeader) {
    ...
    await get(
      ...
      "GET purchase missing → 404",
      false
    );
    await get(
      "/api/credit-cards/00000000-0000-0000-0000-000000000000/current-debt",
      "GET current-debt missing → 404",
      false
    );
    await get(
      "/api/credit-cards/00000000-0000-0000-0000-000000000000/commitments",
      "GET commitments missing → 404",
      false
    );
  }
  ...
  results.push({
    ...
    ok: web.ok,
    detail: `${web.status} ${ORIGIN}`,
  });

  console.log(JSON.stringify({ api: API, web: ORIGIN, results }, null, 2));
